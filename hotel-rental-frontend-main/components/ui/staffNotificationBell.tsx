...
export function StaffNotificationBell() {
  ...
  const { mutate: clearNotifications, isPending: isClearing } = useMutation({
    mutationFn: () => axiosInstance.delete("/system/notifications/staff"),
    onSettled: () => {
  ...
  const { mutate: deleteNotification } = useMutation({
    mutationFn: (id: string) => axiosInstance.delete(`/system/notifications/staff/${id}`),
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ["staff-notifications"] });
    },
  });
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="relative flex size-9 items-center justify-center rounded-xl bg-[#FAF5F5] dark:bg-[#1A0E13] border border-[#D9C3C3] dark:border-white/10 text-[#5C454B] dark:text-gray-300 hover:bg-[#618685]/10 hover:text-[#618685] dark:hover:text-[#88afae] transition-colors cursor-pointer"
        aria-label="Staff notifications"
        title="Staff Alerts"
      >
        <Bell className="size-4.5" />
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 flex size-5 items-center justify-center rounded-full bg-[#618685] text-[10px] font-bold text-white shadow-sm ring-2 ring-white dark:ring-[#130005]">
      ...
      {open && (
        <div className="absolute right-0 top-11 w-[340px] sm:w-[380px] rounded-3xl border border-[#D9C3C3] dark:border-white/10 bg-white dark:bg-[#1A0E13] shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-150">
          {/* Header */}
          <div className="bg-[#130005] p-4 text-white flex items-center justify-between border-b border-[#618685]/40">
            <div className="flex items-center gap-2.5">
              <div className="size-8 rounded-lg bg-[#618685]/30 text-[#88afae] flex items-center justify-center">
                <Sparkles className="size-4" />
              </div>
              <div>
                <h4 className="text-xs font-bold">Staff Alerts</h4>
                <p className="text-[10px] text-[#E4D1D1]/70">
                  {unreadCount > 0
                    ? `${unreadCount} new guest message${unreadCount === 1 ? "" : "s"} waiting`
                    : "You're all caught up"}
                </p>
              </div>
            </div>
            <span className="text-[10px] font-semibold uppercase px-2.5 py-0.5 rounded-full bg-[#618685]/30 text-white border border-[#88afae]/40 inline-flex items-center gap-1.5">
          ...
          {/* Items list */}
          <div className="max-h-[320px] overflow-y-auto divide-y divide-[#D9C3C3]/40 dark:divide-white/10 p-2">
            {chatItems.length === 0 ? (
              <div className="py-10 text-center text-xs text-[#5C454B] dark:text-gray-400">
                <Inbox className="size-9 text-[#618685]/50 mx-auto mb-2" />
                <p className="font-bold text-[#130005] dark:text-white">No Alerts Yet</p>
                <p className="text-[11px] mt-0.5">
                  {unreadCount === 0 && notifications
                    ? "Waiting for guest inquiries to arrive here."
              ...
              chatItems.map((item) => (
                <div key={item.id} className="relative">
                  ...
                  >
                    {!item.read && (
                      <span className="absolute -ml-0.5 size-2 rounded-full bg-[#618685] mt-2 ring-2 ring-white dark:ring-[#1A0E13]" />
                    )}
                    <div
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-1">
                        <h5 className="text-xs font-semibold text-[#130005] dark:text-white truncate group-hover:text-[#618685] dark:group-hover:text-[#88afae] transition-colors">
                          {item.title}
                        </h5>
          ...
          <div className="p-3 bg-[#FAF5F5] dark:bg-[#130005] border-t border-[#D9C3C3] dark:border-white/10 flex items-center justify-between gap-2 text-xs">
            <Link
              href="/pages/staff/chat"
              onClick={() => setOpen(false)}
              className="flex items-center gap-1 text-[11px] font-semibold text-[#618685] dark:text-[#88afae] hover:underline"
            >
              Open Guest Messages
